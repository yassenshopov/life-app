...
export async function POST() {
  try {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: { message: 'Failed to skip to previous track' } }));
      const errorMessage = error.error?.message || 'Failed to skip to previous track';
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to skip to previous track' },
      { status: 500 }
    );
  }
}
