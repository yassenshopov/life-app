...
import { Heart, Scale, Footprints, Moon, LineChart as LineChartIcon, BarChart3 } from 'lucide-react';
import { Card } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { TrackingEntry, formatStoredPropertyValue } from '@/components/TrackingView';
import { ResponsiveContainer, LineChart, Line, AreaChart, Area, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, defs, linearGradient, stop, Cell } from 'recharts';
import { format, subDays, startOfWeek, endOfWeek, addWeeks, subWeeks, startOfMonth, endOfMonth, addMonths, subMonths } from 'date-fns';
...
export function HealthMetricsTrends({ entries, viewMode = 'daily' }: HealthMetricsTrendsProps) {
  ...
  const chartData = useMemo(() => {
    ...
    if (viewMode === 'monthly') {
      // Group by month
      const cutoffDate = selectedMonthsPeriod === 'all' 
        ...
        try {
          ...
          if (sleepValue && typeof sleepValue === 'number' && sleepValue > 0) {
            const deepPercent = deepSleepPercent !== null 
              ...
              : (deepSleepHours !== null && typeof deepSleepHours === 'number' ? (deepSleepHours / sleepValue) * 100 : null);
            if (deepPercent !== null && typeof deepPercent === 'number') monthData.deepSleepValues.push(deepPercent);
            
            const remPercent = remSleepPercent !== null
              ? (remSleepPercent < 1 ? remSleepPercent * 100 : remSleepPercent)
              : (remSleepHours !== null && typeof remSleepHours === 'number' ? (remSleepHours / sleepValue) * 100 : null);
            if (remPercent !== null && typeof remPercent === 'number') monthData.remSleepValues.push(remPercent);
            
            const awakePercent = awakeTime !== null && typeof awakeTime === 'number'
            ...
            const lightPercent = lightSleepPercent !== null
              ...
              : (deepPercent !== null && remPercent !== null && awakePercent !== null
                ...
                : null);
            if (lightPercent !== null && typeof lightPercent === 'number') monthData.lightSleepValues.push(lightPercent);
          }
        } catch {
          // Skip invalid dates
        }
      ...
      return Array.from(monthsMap.values())
        ...
        .map((monthData) => {
          const rhr = monthData.rhrValues.length > 0
            ? Math.round(monthData.rhrValues.reduce((a, b) => a + b, 0) / monthData.rhrValues.length)
            : null;
          const weight = monthData.weightValues.length > 0
            ? Math.round((monthData.weightValues.reduce((a, b) => a + b, 0) / monthData.weightValues.length) * 100) / 100
            : null;
          const steps = monthData.stepsValues.length > 0
            ? Math.round(monthData.stepsValues.reduce((a, b) => a + b, 0) / monthData.stepsValues.length)
            : null;
          const sleep = monthData.sleepValues.length > 0
            ? Math.round((monthData.sleepValues.reduce((a, b) => a + b, 0) / monthData.sleepValues.length) * 100) / 100
            : null;
          const deepSleep = monthData.deepSleepValues.length > 0
            ? Math.round((monthData.deepSleepValues.reduce((a, b) => a + b, 0) / monthData.deepSleepValues.length) * 100) / 100
            : null;
          const lightSleep = monthData.lightSleepValues.length > 0
            ? Math.round((monthData.lightSleepValues.reduce((a, b) => a + b, 0) / monthData.lightSleepValues.length) * 100) / 100
            : null;
          const remSleep = monthData.remSleepValues.length > 0
            ? Math.round((monthData.remSleepValues.reduce((a, b) => a + b, 0) / monthData.remSleepValues.length) * 100) / 100
            : null;
          const awake = monthData.awakeValues.length > 0
            ? Math.round((monthData.awakeValues.reduce((a, b) => a + b, 0) / monthData.awakeValues.length) * 100) / 100
            : null;
      ...
      // Group by week
      const cutoffDate = selectedWeeksPeriod === 'all'
        ? new Date(0) // Very old date to include all data
        ...
        try {
          ...
          if (sleepValue && typeof sleepValue === 'number' && sleepValue > 0) {
            const deepPercent = deepSleepPercent !== null 
              ? (deepSleepPercent < 1 ? deepSleepPercent * 100 : deepSleepPercent)
              : (deepSleepHours !== null && typeof deepSleepHours === 'number' ? (deepSleepHours / sleepValue) * 100 : null);
            if (deepPercent !== null && typeof deepPercent === 'number') weekData.deepSleepValues.push(deepPercent);
            
            const remPercent = remSleepPercent !== null
              ? (remSleepPercent < 1 ? remSleepPercent * 100 : remSleepPercent)
              : (remSleepHours !== null && typeof remSleepHours === 'number' ? (remSleepHours / sleepValue) * 100 : null);
            if (remPercent !== null && typeof remPercent === 'number') weekData.remSleepValues.push(remPercent);
            ...
            const lightPercent = lightSleepPercent !== null
              ...
              : (deepPercent !== null && remPercent !== null && awakePercent !== null
                ? Math.max(0, 100 - deepPercent - remPercent - (awakePercent || 0))
                : null);
            if (lightPercent !== null && typeof lightPercent === 'number') weekData.lightSleepValues.push(lightPercent);
          }
        } catch {
          // Skip invalid dates
        }
      ...
      return Array.from(weeksMap.values())
        ...
        .map((weekData) => {
          const rhr = weekData.rhrValues.length > 0
            ? Math.round(weekData.rhrValues.reduce((a, b) => a + b, 0) / weekData.rhrValues.length)
            : null;
          const weight = weekData.weightValues.length > 0
            ? Math.round((weekData.weightValues.reduce((a, b) => a + b, 0) / weekData.weightValues.length) * 100) / 100
            : null;
          const steps = weekData.stepsValues.length > 0
            ? Math.round(weekData.stepsValues.reduce((a, b) => a + b, 0) / weekData.stepsValues.length)
            : null;
          const sleep = weekData.sleepValues.length > 0
            ? Math.round((weekData.sleepValues.reduce((a, b) => a + b, 0) / weekData.sleepValues.length) * 100) / 100
            : null;
          const deepSleep = weekData.deepSleepValues.length > 0
            ? Math.round((weekData.deepSleepValues.reduce((a, b) => a + b, 0) / weekData.deepSleepValues.length) * 100) / 100
            : null;
          const lightSleep = weekData.lightSleepValues.length > 0
            ? Math.round((weekData.lightSleepValues.reduce((a, b) => a + b, 0) / weekData.lightSleepValues.length) * 100) / 100
            : null;
          const remSleep = weekData.remSleepValues.length > 0
            ? Math.round((weekData.remSleepValues.reduce((a, b) => a + b, 0) / weekData.remSleepValues.length) * 100) / 100
            : null;
          const awake = weekData.awakeValues.length > 0
            ? Math.round((weekData.awakeValues.reduce((a, b) => a + b, 0) / weekData.awakeValues.length) * 100) / 100
            : null;
      ...
      const cutoffDate = selectedDaysPeriod === 'all'
        ? new Date(0) // Very old date to include all data
      ...
      return filteredEntries.map((e) => {
        ...
        if (sleep !== null && sleep > 0) {
          deepSleep = deepSleepPercent !== null 
            ? (deepSleepPercent < 1 ? deepSleepPercent * 100 : deepSleepPercent)
            : (deepSleepHours !== null && typeof deepSleepHours === 'number' ? (deepSleepHours / sleep) * 100 : null);
          
          remSleep = remSleepPercent !== null
            ? (remSleepPercent < 1 ? remSleepPercent * 100 : remSleepPercent)
            : (remSleepHours !== null && typeof remSleepHours === 'number' ? (remSleepHours / sleep) * 100 : null);
          
          awake = awakeTime !== null && typeof awakeTime === 'number'
            ? (awakeTime / 60 / sleep) * 100
            : null;
          
          lightSleep = lightSleepPercent !== null
            ? (lightSleepPercent < 1 ? lightSleepPercent * 100 : lightSleepPercent)
  ...
  }, [entries, selectedDaysPeriod, selectedWeeksPeriod, selectedMonthsPeriod, viewMode]);
  ...
  const hasSleepStages = chartData.some(d => 
    d.deepSleep !== null || d.lightSleep !== null || d.remSleep !== null || d.awake !== null
  );
                ...
                {chartType === 'line' ? (
                  <AreaChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                    ...
                    <YAxis 
                      ...
                      tick={{ fontSize: 10 }}
                      label={{ value: 'h', angle: -90, position: 'insideLeft', style: { fontSize: 10 } }}
                    />
                    <Tooltip
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
            ...
            <div className="space-y-3">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                  ...
                  />
                  <Bar dataKey="deepSleep" stackId="sleep" fill="#2563eb" radius={[0, 0, 0, 0]} />
                  <Bar dataKey="lightSleep" stackId="sleep" fill="#eab308" radius={[0, 0, 0, 0]} />
                  <Bar dataKey="remSleep" stackId="sleep" fill="#ec4899" radius={[0, 0, 0, 0]} />
